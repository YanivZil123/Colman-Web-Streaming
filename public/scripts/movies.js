(async()=>{
    ...
    function initHorizontalScroll(grid) {
      ...
      async function loadMoreContent() {
        ...
        try {
          const response = await api.get(`/api/titles?genre=${encodeURIComponent(genre)}&type=movie&page=${currentPage}&limit=${contentLimit}`);
    ...
    async function loadGenreContent(genreSlug) {
      try {
  const response = await api.get(`/api/titles?genre=${encodeURIComponent(genreSlug)}&type=movie&limit=${contentLimit}`);
        const data = await response.json();
