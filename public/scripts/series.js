(async()=>{
    ...
    function initHorizontalScroll(grid, initialItems = [], genreSlug) {
      ...
      async function loadMoreContent() {
        ...
        try {
          const response = await api.get(`/api/titles?genre=${encodeURIComponent(genreSlug)}&type=series&page=${currentPage}&limit=${contentLimit}&sort=popularity`);
          const data = await response.json();
          ...
          if (items.length === 0) {
            allItemIds.clear();
            currentPage = 1;
            const resetResponse = await api.get(`/api/titles?genre=${encodeURIComponent(genreSlug)}&type=series&page=1&limit=${contentLimit}&sort=popularity`);
    ...
    function createContentCard(t) {
      ...
      card.innerHTML = `
        <div class="content-thumbnail">
          <img src="${resolveImageUrl(t.thumbnailUrl || t.posterUrl)}" alt="${t.name || 'Series'}" onerror="this.src='/images/poster-placeholder.jpg'">
